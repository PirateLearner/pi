...
//		console.log($('.comments-form-block'));
	...
	var postAnnotation = function(e){
		...
		data = {
				body: body,
				paragraph: id,
				content_type: annotations.content_type,
				object_id: annotations.id,
				privacy: "3",
				privacy_override:"0",
				shared_with: [],
			};
	    ...
	    $.ajax({
	        cache: false,
	        url : window.location.origin+"/en/rest/annotations/",
	        type: "POST",
	        dataType : "json",
...
//		console.log('Binding complete');
	 ...
	 */	
	annotations.id = parseInt($('.rest').attr('data-id'));
	annotations.content_type = parseInt($('.rest').attr('data-content-type'));
	/*

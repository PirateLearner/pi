		...
		if (data.result == 'success') {
			...
			if (data.action == 'Delete')			
			{
				...
				for (var i = 0; i < data.published_id.length; i++) {
					...
					console.log($("td").find('[data-prom-id="' + data.published_id[i] + '"]').html());
					$("td").find('[data-prom-id="' + data.published_id[i] + '"]').html('<i class="material-icons mdl-color-text--green-600" title="promoted">check</i>');
				}
			}
				...
				for (var i = 0; i < data.published_id.length; i++) {
					console.log(data.published_id[i]);
					console.log($("td").find('[data-prom-id="' + data.published_id[i] + '"]').html());
					$("td").find('[data-prom-id="' + data.published_id[i] + '"]').html('<i class="material-icons mdl-color-text--red-300" title="Not promoted" >close</i>');
				}
			}
			else if (data.action == 'Public') {
				/* Change the icons of public icons */
				console.log(data.published_id)
				for (var i = 0; i < data.published_id.length; i++) {
					console.log(data.published_id[i]);
					console.log($("td").find('[data-priv-id="' + data.published_id[i] + '"]').html());
					$("td").find('[data-priv-id="' + data.published_id[i] + '"]').html('<i class="material-icons mdl-color-text--blue-600" title="public">people</i>');
				}
			}
			else if (data.action == 'Private') {
				/* Change the icons of private icons */
				console.log(data.published_id)
				for (var i = 0; i < data.published_id.length; i++) {
					console.log(data.published_id[i]);
					console.log($("td").find('[data-priv-id="' + data.published_id[i] + '"]').html());
					$("td").find('[data-priv-id="' + data.published_id[i] + '"]').html('<i class="material-icons" title="private">person</i>');
				}
			}

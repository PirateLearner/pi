...
from annotation.models import get_annotation_for_model

register = template.Library()


@register.inclusion_tag("annotation/templatetags/ajax_annotation_tags.html", takes_context=True)
...
class AnnotationList(Node):
    def render(self, context):
        ...
        if not getattr(self, 'nodelist', None):
            template = get_template("annotation/templatetags/flat_list.html")
...
class BaseAnnotationNode(template.Node):
    ...
    def __init__(self, ctype=None, object_pk_expr=None, object_expr=None, as_varname=None, body=None):
        ...
        self.comment_model = annotation.get_model()
...
class AnnotationFormNode(BaseAnnotationNode):
    ...
    def get_form(self, context):
        ...
        if obj:
            return annotation.get_form()(obj)
        else:
            return None
...
class RenderAnnotationFormNode(AnnotationFormNode):
    """Render the annotation form directly"""
    ...
    def render(self, context):
        ...
        if object_pk:
            template_search_list = [
                "annotation/%s/%s/form.html" % (ctype.app_label, ctype.model),
                "annotation/%s/form.html" % ctype.app_label,
...
class RenderAnnotationListNode(AnnotationListNode):
    """Render the annotation list directly"""
    ...
    def render(self, context):
        ...
        if object_pk:
            template_search_list = [
                "annotation/%s/%s/list.html" % (ctype.app_label, ctype.model),
                "annotation/%s/list.html" % ctype.app_label,
...
def get_annotation_count(parser, token):
    """
    Gets the annotation count for the given params and populates the template
...
def render_annotation_list(parser, token):
    """
    Render the annotation list (as returned by ``{% get_annotation_list %}``)
    through the ``annotation/list.html`` template
...
def render_annotation_form(parser, token):
    """
    Render the annotation form (as returned by ``{% render_annotation_form %}``) through
    the ``annotation/form.html`` template.
...
def annotation_form_target():
    """
    Get the target URL for the annotation form.

    Example::

        <form action="{% annotation_form_target %}" method="post">
    """
    return annotation.get_form_target()

@register.simple_tag
def get_annotation_permalink(comment, anchor_pattern=None):
    """
    Get the permalink for an annotation, optionally specifying the format of the
    named anchor to be appended to the end of the URL.

    Example::
        {% get_annotation_permalink annotation "#c%(id)s-by-%(user_name)s" %}
    """

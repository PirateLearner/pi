...
class AnnotationPostBadRequest(http.HttpResponseBadRequest):
    ...
    def __init__(self, why):
        ...
        if settings.DEBUG:
            self.content = render_to_string("annotation/400-debug.html", {"why": why})
...
def post_annotation(request, next=None, using=None):
    ...
    #data is the comment
    form = annotation.get_form()(target, data=data)
    ...
    if form.errors:
        template_list = [
            ...
            "annotation/%s/%s/preview.html" % (model._meta.app_label, model._meta.module_name),
            "annotation/%s/preview.html" % model._meta.app_label,
    ...
    # To create an annotation, the user must be logged in, thus that field must
    ...
    return next_redirect(request, fallback=next or 'annotation:annotation-comment-done',
...
def post_annotation_ajax(request, using=None):
    """
    Post an annotation, via an Ajax call.
...
def _ajax_result(request, form, action, comment=None, object_id=None):
    ...
    if comment is not None:
        ...
        comment_html = render_to_string('annotation/annotation.html', context, context_instance=RequestContext(request))
...
annotation_done = confirmation_view(
    template="annotation/posted.html",
    doc="""Display a "annotation was posted" success page."""
)
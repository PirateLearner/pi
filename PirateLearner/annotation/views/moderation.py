...
@csrf_protect
@permission_required("annotation.can_moderate")
def delete(request, comment_id, next=None):
    ...
    """
    comment = get_object_or_404(annotation.get_model(), pk=comment_id, site__pk=settings.SITE_ID)

    # Delete on POST
    if request.method == 'POST':
        # Flag the comment as deleted instead of actually deleting it.
        perform_delete(request, comment)
        return next_redirect(request, fallback=next or 'annotation-delete-done',
            c=comment.pk)

    # Render a form on GET
    else:
        return render_to_response('annotation/delete.html',
...
def approve(request, comment_id, next=None):
    ...
    Context:
        body
            the `annotation.annotation` object for approval
    """
    comment = get_object_or_404(annotation.get_model(), pk=comment_id, site__pk=settings.SITE_ID)
    ...
    if request.method == 'POST':
        ...
        return next_redirect(request, fallback=next or 'annotation-approve-done',
            c=comment.pk)

    # Render a form on GET
    else:
        return render_to_response('annotation/approve.html',
            {'body': comment, "next": next},
            template.RequestContext(request)
        )
...
def perform_delete(request, comment):
    '''
    flag, created = annotation.models.CommentFlag.objects.get_or_create(
        comment = comment,
        user    = request.user,
        flag    = annotation.models.CommentFlag.MODERATOR_DELETION
    )
    '''
